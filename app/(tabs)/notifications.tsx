...
const skSeenUpdates = (uid: string) => `seen_request_updates_v1_${uid}`;
...
function buildNotifications(
  ...
  if (!todayLogged) {
    const existingReminder = stored.find(
      (n) => n.type === "reminder" && n.id === "auto-daily",
    );
    generated.push({
      id: "auto-daily",
      type: "reminder",
  ...
  if (entries.length >= 7) {
    ...
    const moodLabel =
      avgMood > 0
        ? getMoodConfig(Math.round(avgMood) as MoodLevel).label
        : "Neutral";
    generated.push({
      id,
      type: "summary",
  ...
  const manualStored = stored.filter(
    (n) => !n.id.startsWith("auto-") && !autoIds.has(n.id),
  );

  return [...generated, ...manualStored].sort(
...
export default function NotificationsScreen() {
  ...
  const [reminders, setReminders] =
    useState<ReminderSettings>(DEFAULT_REMINDERS);
  const [loaded, setLoaded] = useState(false);
  const [friendRequests, setFriendRequests] = useState<FriendRequest[]>([]);
  const [requestUpdates, setRequestUpdates] = useState<FriendRequest[]>([]);
  const [seenUpdateIds, setSeenUpdateIds] = useState<Set<string>>(new Set());
  const [socialNotifications, setSocialNotifications] = useState<
    Notification[]
  >([]);

  // Load persisted read-state, reminder settings, pending requests, and request updates
  useEffect(() => {
    if (!user?.id) return;
    const uid = user.id;
    setLoaded(false);
    (async () => {
      try {
        const [nb, rb, seenRaw] = await Promise.all([
          AsyncStorage.getItem(skNotifs(uid)),
          AsyncStorage.getItem(skReminders(uid)),
          AsyncStorage.getItem(skSeenUpdates(uid)),
        ]);
        if (nb) setStoredNotifs(JSON.parse(nb));
        if (rb) setReminders(JSON.parse(rb));
        if (seenRaw) setSeenUpdateIds(new Set(JSON.parse(seenRaw)));
      } catch {}
      setLoaded(true);
      try {
        const [requests, updates, socialNotifs] = await Promise.all([
          getPendingRequests(uid),
          getRequestUpdates(uid),
          getNotifications(uid),
        ]);
        setFriendRequests(requests);
        setRequestUpdates(updates);
        setSocialNotifications(socialNotifs);
      } catch {}
    })();
  }, [user?.id]);
  ...
  const visibleUpdates = requestUpdates.filter((r) => !seenUpdateIds.has(r.id));
  ...
  function markRead(id: string) {
    ...
    const stored = updated.map(({ id, read, timestamp }) => ({
      id,
      read,
      timestamp,
    }));
    ...
    if (uid)
      AsyncStorage.setItem(skNotifs(uid), JSON.stringify(stored)).catch(
        () => {},
      );
  }
  ...
  async function handleFriendRequestResponse(
    ...
    try {
      ...
      if (status === "accepted") {
        Alert.alert(
          "Connected!",
          `You are now connected with ${request.senderProfile?.fullName || "this user"} as ${request.relationshipType}. They will appear in your ${request.relationshipType === "family" ? "Family" : "Friends"} circle.`,
        );
      }
    } catch {
  ...
  async function handleRequestUpdateDismiss(request: FriendRequest) {
    // For rejected requests: delete the DB row so it never surfaces again
    ...
    // Mark as seen so it won't reappear on next load
    const next = new Set(seenUpdateIds);
    next.add(request.id);
    setSeenUpdateIds(next);
    setRequestUpdates((prev) => prev.filter((r) => r.id !== request.id));
    if (uid)
      AsyncStorage.setItem(skSeenUpdates(uid), JSON.stringify([...next])).catch(
        () => {},
      );
  }
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top"]}>
      ...
      >
        ...
        {activeTab === "activity" && (
          <>
            {/* Request Updates — responses to requests User A sent */}
            {visibleUpdates.length > 0 && (
              <>
                <Text style={styles.sectionLabel}>Request Updates</Text>
                <View style={styles.notifList}>
                  {visibleUpdates.map((req) => {
                    const accepted = req.status === "accepted";
                    const name = req.receiverProfile?.fullName || "Someone";
                    const circle =
                      req.relationshipType === "family" ? "Family" : "Friends";
                    return (
                      <View key={req.id} style={styles.requestCard}>
                        <View style={styles.requestLeft}>
                          ...
                          >
                            <Ionicons
                              name={
                                accepted ? "checkmark-circle" : "close-circle"
                              }
                              size={22}
                          ...
                          <View style={styles.requestInfo}>
                            ...
                            <Text style={styles.requestSub}>
                              {accepted
                                ? `accepted your ${req.relationshipType} request — now in your ${circle} circle`
                                : `declined your ${req.relationshipType} request`}
                            </Text>
                          </View>
                        </View>
                        <TouchableOpacity
                          style={styles.requestDecline}
                          onPress={() => handleRequestUpdateDismiss(req)}
                        >
                          <Text style={styles.requestDeclineText}>Dismiss</Text>
                        </TouchableOpacity>
                      </View>
                    );
                  })}
            ...
            {/* Social Notifications (removals, etc.) */}
                      ...
                      <View style={styles.requestLeft}>
                        <View style={styles.requestAvatar}>
                          <Text style={styles.requestAvatarText}>
                            {(notif.fromUserProfile?.fullName || "U")
                              .slice(0, 2)
                              .toUpperCase()}
                          </Text>
                        </View>
                        <View style={styles.requestInfo}>
                          <Text style={styles.requestName}>
                            {notif.fromUserProfile?.fullName || "Someone"}
                          </Text>
                          <Text style={styles.requestSub}>{notif.message}</Text>
                        </View>
                      </View>
                      <TouchableOpacity
                        style={styles.requestDecline}
                  ...
                  {friendRequests.map((req) => (
                    <View key={req.id} style={styles.requestCard}>
                      <View style={styles.requestLeft}>
                        ...
                        <View style={styles.requestInfo}>
                          <Text style={styles.requestName}>
                            {req.senderProfile?.fullName || "Unknown"}
                          </Text>
                          <Text style={styles.requestSub}>
                            wants to add you as{" "}
                            {req.relationshipType === "family"
                              ? "Family"
                              : "a Friend"}
                          </Text>
                        </View>
                      ...
                      <View style={styles.requestActions}>
                        <TouchableOpacity
                          ...
                          onPress={() =>
                            handleFriendRequestResponse(req, "accepted")
                          }
                        >
                        ...
                        <TouchableOpacity
                          style={styles.requestDecline}
                          onPress={() =>
                            handleFriendRequestResponse(req, "rejected")
                          }
                        >
            ...
            {notifications.length === 0 ? (
              <View style={styles.emptyState}>
                ...
                <Text style={styles.emptyBody}>
                  Log your mood daily to start seeing streak alerts, insights,
                  and summaries here.
                </Text>
              </View>
            ) : (
                ...
                {aiInsight && (
                  ...
                  >
                    <View style={styles.aiCardBadge}>
                      ...
                      <Text style={styles.aiCardBadgeText}>
          ...
          <>
            ...
            <View style={styles.statsRow}>
              ...
              <StatCard
                ...
                value={
                  avgMood > 0
                    ? getMoodConfig(Math.round(avgMood) as MoodLevel).emoji
                    : "—"
                }
                label="7-day avg"
              />
            ...
            >
              <View
                style={[styles.settingIcon, { backgroundColor: Colors.border }]}
              >
                <Ionicons
        ...
        {activeTab === "tips" && (
          <>
            <View style={styles.tipsIntro}>
              <Text style={styles.tipsIntroTitle}>
                Get the most from MoodBoard
              </Text>
              <Text style={styles.tipsIntroBody}>
                These practices — based on mood-tracking research — help you
                build a habit that actually reveals patterns.
              </Text>
            </View>
